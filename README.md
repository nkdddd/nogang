...
- **학습시간은 타이머로 잰 시간만 인정**: 학습 카드의 ▶ 타이머, 포커스 타이머로 잰 시간(`timerMin`)만 학습시간·용돈·캐릭터 성장에 들어갑니다.
  손으로 적은 시간은 기록으로만 남고 0분. (학습앱 인정 시간은 앱이 직접 잰 기록이라 그대로 반영)
- **대신 관대하게 지급**: 주간 목표시간 = 하루 목표 합계 × 60%, 난이도 커브 1.15, 최소 요건 주 180분·3일(미달 감액 완화),
  복습 목표 3회·학습일 목표 5일, 초과 학습 보너스 1시간당 1,000원(최대 8,000원).
- 이미 쓰던 계정은 처음 열 때 한 번 자동으로 올라갑니다. 부모가 직접 바꿔 둔 값(상한·가중치 등)은 그대로 둡니다.
