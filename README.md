...
payTerms/{자녀uid}              💰 용돈 기준 (1시간당 금액 등) — 연결된 부모만 쓰기
...
## 💰 용돈 = 타이머로 공부한 시간 × 1시간당 금액

- **학습시간은 타이머로 잰 시간만 인정**: 학습 카드의 ▶ 타이머, 포커스 타이머(과제 선택 · 과제 없이 모두)로 잰 시간.
  손으로 적은 시간은 기록으로만 남고 0분. 학습앱(천문장 · 영단어 · 문법)이 잰 시간은 부모가 포함/제외를 고릅니다.
- **금액은 부모가 정함**: `payTerms/{자녀uid}` = {1시간당 금액(기본 1,000원), 주간 최대(기본 20,000원), 하루 최대 인정(기본 4시간), 학습앱 포함}
  - 부모: 기록 → 용돈 정산(자녀별로 바로 수정) 또는 자녀 플래너를 연 상태에서 설정 → 용돈 기준
  - 자녀 화면에는 기준이 보이기만 하고, 보안 규칙상 연결된 부모만 바꿀 수 있습니다.
- 지급할 때 금액을 직접 고칠 수 있고, 시험 대비 부스터 기간에는 배율이 곱해집니다.
- 계획 완료 · 학습일 · 복습 · 정답률은 참고 지표로만 보여 주고 금액에는 넣지 않습니다.
